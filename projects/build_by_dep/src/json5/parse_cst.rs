                        ...
                        s.sequence(|s| {
                            parse_object_pair(s)
                                ...
                                .and_then(|s| {
                                    s.optional(|s| {
                                        s.sequence(|s| {
                                            s.sequence(|s| builtin_text::<false>(s, ",").and_then(|s| builtin_ignore(s)))
...
fn parse_string(state: Input) -> Output {
    state.rule(Json5Rule::String, |s| {
        s.sequence(|s| {
            builtin_text::<false>(s, "'").and_then(|s| parse_string_raw(s)).and_then(|s| builtin_text::<false>(s, "'"))
        })
        .or_else(|s| {
            s.sequence(|s| {
                builtin_text::<false>(s, "'")
                    .and_then(|s| s.optional(|s| parse_string_text(s)))
                    .and_then(|s| builtin_text::<false>(s, "'"))
            })
        })
    })
...
fn parse_string_text(state: Input) -> Output {
    state.rule(Json5Rule::StringText, |s| {
        s.sequence(|s| builtin_text::<false>(s, "\\").and_then(|s| builtin_any(s))).or_else(|s| {
            builtin_regex(s, {
                static REGEX: OnceLock<Regex> = OnceLock::new();
                REGEX.get_or_init(|| Regex::new("^([^\"]+)").unwrap())
            })
