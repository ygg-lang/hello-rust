...
fn parse_value(state: Input) -> Output {
    state.rule(Json5Rule::Value, |s| {
        parse_object(s).and_then(|s| s.tag_node("object")).or_else(|s|parse_array(s).and_then(|s| s.tag_node("array"))).or_else(|s|parse_string(s).and_then(|s| s.tag_node("string"))).or_else(|s|parse_number(s).and_then(|s| s.tag_node("number"))).or_else(|s|parse_boolean(s).and_then(|s| s.tag_node("boolean"))).or_else(|s|parse_null(s).and_then(|s| s.tag_node("null")))
    })
}
#[inline]
fn parse_object(state: Input) -> Output {
    state.rule(Json5Rule::Object, |s| {
        s.sequence(|s|builtin_text(s, "{", false).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|s.sequence(|s|parse_object_pair(s).and_then(|s| s.tag_node("object_pair")).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295, |s|s.sequence(|s|s.sequence(|s|builtin_text(s, ",", false).and_then(|s|builtin_ignore(s))).and_then(|s|parse_object_pair(s).and_then(|s| s.tag_node("object_pair")))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, ",", false))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, "}", false)))
    })
}
#[inline]
fn parse_object_pair(state: Input) -> Output {
    state.rule(Json5Rule::ObjectPair, |s| {
        s.sequence(|s|s.sequence(|s|parse_object_key(s).and_then(|s| s.tag_node("object_key")).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, ":", false)).and_then(|s|builtin_ignore(s))).and_then(|s|parse_value(s).and_then(|s| s.tag_node("value"))))
    })
}
#[inline]
fn parse_object_key(state: Input) -> Output {
    state.rule(Json5Rule::ObjectKey, |s| {
        parse_identifier(s).and_then(|s| s.tag_node("identifier")).or_else(|s|parse_string(s).and_then(|s| s.tag_node("string")))
    })
}
#[inline]
fn parse_array(state: Input) -> Output {
    state.rule(Json5Rule::Array, |s| {
        s.sequence(|s|builtin_text(s, "[", false).and_then(|s|builtin_ignore(s)).and_then(|s|s.optional(|s|s.sequence(|s|parse_value(s).and_then(|s| s.tag_node("value")).and_then(|s|builtin_ignore(s)).and_then(|s|s.repeat(0..4294967295, |s|s.sequence(|s|s.sequence(|s|builtin_text(s, ",", false).and_then(|s|builtin_ignore(s))).and_then(|s|parse_value(s).and_then(|s| s.tag_node("value")))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, ",", false))))).and_then(|s|builtin_ignore(s)).and_then(|s|builtin_text(s, "]", false)))
    })
}
#[inline]
fn parse_string(state: Input) -> Output {
    state.rule(Json5Rule::String, |s| {
        s.sequence(|s|builtin_text(s, "'", false).and_then(|s|parse_string_raw(s).and_then(|s| s.tag_node("string_raw"))).and_then(|s|builtin_text(s, "'", false))).or_else(|s|s.sequence(|s|builtin_text(s, "'", false).and_then(|s|s.repeat(0..4294967295, |s|parse_string_text(s).and_then(|s| s.tag_node("string_text")))).and_then(|s|builtin_text(s, "'", false))))
    })
}
#[inline]
...
fn parse_string_text(state: Input) -> Output {
    state.rule(Json5Rule::StringText, |s| {
        parse_string_escape(s).and_then(|s| s.tag_node("string_escape")).or_else(|s|builtin_regex(s,{static REGEX:OnceLock<Regex>=OnceLock::new();REGEX.get_or_init(||Regex::new("^([^\"]+)").unwrap())}))
    })
}
#[inline]
fn parse_string_escape(state: Input) -> Output {
    state.rule(Json5Rule::StringEscape, |s| {
        s.sequence(|s|s.sequence(|s|builtin_text(s, "\\", false).and_then(|s|builtin_ignore(s))).and_then(|s|builtin_any(s).and_then(|s| s.tag_node("c"))))
    })
}
#[inline]
...
fn parse_boolean(state: Input) -> Output {
    state.rule(Json5Rule::Boolean, |s| {
        builtin_text(s, "true", false).or_else(|s|builtin_text(s, "false", false))
    })
}
#[inline]
...
fn parse_comment(state: Input) -> Output {
    state.rule(Json5Rule::Comment, |s| {
        Err(/*comment_line*/s).or_else(|s|Err(/*comment_line*/s)).or_else(|s|Err(/*comment_block*/s))
    })
}
